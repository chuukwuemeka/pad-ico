...
const Home: NextPage = () => {
  ...
  const sliderBG = useColorModeValue('blue.500', 'blue.300');
  const filled = useColorModeValue('gray.300', 'gray.600');
  
  // React.useState
  const [refresh, setRefresh] = React.useState<boolean>(true);
  const [account, setAccount] = React.useState<any>(null);
  const [contractPAD, setContractPAD] = React.useState<any>(null);
  const [contractPTS, setContractPTS] = React.useState<any>(null);
  ...
  const [PADtoETH, setPADtoETH] = React.useState<any>(null);
  const [myPAD, setMyPAD] = React.useState<number>(0);
  ...
  const handleUseEffect = async () => {
    ...
    load().then((e) => {
      ...
      setContractPAD(e.contractPAD);
      setContractPTS(e.contractPTS);
      setTokensSold(e.tokensSold);
      setTransactionCount(e.transactionCount);
      setEthFunds(e.ethFunds);
      const padETH = (10 / e.ethPriceN).toFixed(2);
      setPADtoETH(padETH);
  ...
  // Functions
  const isLoading = () => account == null || contractPAD == null || contractPTS == null || tokensSold == null || transactionCount == null || ethFunds == null || PADtoETH == null;
  const SliderValue = () => {
  ...
  const buyTokens = async () => {
    const big = BigInt(inputValue * 10**18);
      await contractPTS.buyToken(big, {
        from: account,
        value: inputValue * PADtoETH * 10**18,
  ...
  return (
    ...
    <VStack minH='800px' w='full' bg={bg}>
      ...
      <HStack w='md' display={{lg: 'flex', md: 'flex', sm: 'none', base: 'none'}}>
        <Heading size='md'>1PAD = 10 USD = {PADtoETH} ETH</Heading>
        <Spacer/>
        <Link href='https://faucets.chain.link/' isExternal>
          <Button bg='gray.600' color='gray.100' variant='link' w='100px' minH='40px'>Get Kovan</Button>
        </Link>
      </HStack>
      {/* Mobile */}
      ...
      <HStack w='md' display={{lg: 'flex', md: 'flex', sm: 'none', base: 'none'}}>
        <Slider value={SliderValue()} h='50px'>
          <SliderTrack bg={sliderBG} h='25px' borderRadius={8}>
            ...
            <Center>
              <Text>{Number(tokensSold).toFixed()} / 750.000.000 PAD</Text>
            </Center>
          </SliderTrack>
        </Slider>
      </HStack>
      ...
      <Button bg='blue.500' onClick={ () => setIsOpen(true) }>BUY PAD</Button>
      <Box h='10px' />
      {/* Desktop */}
      ...
      <HStack w='md' h='50px' bg={hBG} borderRadius={8} display={{lg: 'flex', md: 'flex', sm: 'none', base: 'none'}}>
        ...
        boxSize='50px'
        />
        <Text>{(ethFunds / 10**18).toFixed(2)} ETH Available</Text>
      </HStack>
      {/* Mobile */}
      <HStack w='full'  display={{lg: 'none', md: 'none', sm: 'flex', base: 'flex'}}>
        <Box w='10px'/>
      ...
      <HStack w='full' display={{lg: 'none', md: 'none', sm: 'flex', base: 'flex'}}>
        ...
        <Image
          ...
          boxSize='50px'
        />
          <Text fontSize='xs'>Address: {account}</Text>
        </HStack>
        <Box w='10px'/>
      </HStack>
      {/* Desktop */}
      <HStack w='md' h='50px' bg={hBG} borderRadius={8} display={{lg: 'flex', md: 'flex', sm: 'none', base: 'none'}}>
        <Image
          ...
          boxSize='50px'
        />
        <Text fontSize='xs'>PAD Balance: {myPAD}</Text>
      </HStack>
      {/* Mobile */}
      <HStack w='full' display={{lg: 'none', md: 'none', sm: 'flex', base: 'flex'}}>
        <Box w='10px' />
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Buy PAD</ModalHeader>
          <ModalCloseButton/>
          <ModalBody>
            <Input
            placeholder='1 PAD'
...
const HeaderFT = () => {
  return (
    <VStack>
       <Head>
        <title>PAD Token ICO</title>
        <meta name="description" content="PadTokenICO" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <Box h='5px'/>
      <HStack w='full'>
        <Spacer />
        <Heading size='2xl'>PAD Token ICO</Heading>
        <Spacer />
      </HStack>
      <Box h='10px'/>
    </VStack>
